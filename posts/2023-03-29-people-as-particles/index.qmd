...

{
    ...
    const containerLegend = container.append("div")
        .attr("style", "display: flex; justify-content: center");
    containerLegend.call(legend);
    
    const containerChart = container.append("div")
        .attr("style", "display: flex; justify-content: space-between");
    containerChart.call(particles, citySelectPH, "10px");
...

{
    const dummynodes = Array.from(
        { length: 200 }, (_, i) => ({
            x: d3.randomUniform((width * .75 * .05), (width * .75 * .95))(),
    ...
    dummynodes.push(
        { x: width * .75 * .05, y: height * .95, r: 7, vx: 1, vy: -1, color: "#4889ab" },
        { x: width * .75 * .95, y: height * .05, r: 7, vx: -1, vy: 1, color: "#C85B89" }
    );
    ...
    const simulation = d3.forceSimulation(nodes)
        ...
        .force("surface", d3.forceSurface()
            .surfaces(bbox([[0, 0], [width * .75, height]]))
            .oneWay(true)
            .radius(d => d.r + 1))
        .force('limit', d3.forceLimit()
            .x0(0).x1(width * .75).y0(0).y1(height))
        .alphaDecay(0)
        .velocityDecay(0)
...
<p style="text-align:center">We're just two lost souls swimming in a fish bowl<br><span class="fade-text" style="animation-name:change-opacity-1;">year</span> <span class="fade-text" style="animation-name:change-opacity-2;">after</span> <span class="fade-text" style="animation-name:change-opacity-3;">year</span></p>
<p style="text-align:center">`r end_mark()`</p>

<br>
<hr>
**Appendix: D3 / Observable code**
...
// Import data
density = FileAttachment('../../datasets/density/density.csv').csv({ typed: true });
densityManila = density.filter(d => d.country === "Philippines");
densityOthers = density.filter(d => d.country !== "Philippines");
groupAssign = FileAttachment('../../datasets/density/group_assign.csv').csv({ typed: true });
...
function particles(selection, city, margin) {
    ...
    if (margin !== undefined) svg.attr("style", `margin-right: ${margin}`);
